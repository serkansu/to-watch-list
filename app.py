...
if query:
    ...
    if results:
        ...
        for r in results:
            ...
            imdb_id = requests.get(external_ids_url).json().get("imdb_id")
            r["imdb_id"] = imdb_id
        ...
        for r in results:
            ...
            with cols[1]:
                ...
                form_key = f"form_{imdb_id or tmdb_id}_{title.replace(' ', '_')}"
...
if movies_data:
    ...
    for i, (imdb_id, movie) in enumerate(sorted_movies, start=1):
        movie_key = imdb_id or movie.get("title", "").replace(" ", "_")
        cols = st.columns([1, 3])
        with cols[0]:
            if movie.get("poster"):
        ...
        with cols[1]:
            ...
            new_priority = st.slider(f"🎛️ Öncelik:", 1, 100, movie["priority"], key=f"edit_{movie_key}")
            col1, col2, col3 = st.columns(3)
            with col1:
                if st.button("📎 Güncelle", key=f"save_{movie_key}"):
                    ref.child(f"to_watch_firebase/{db_key}/{imdb_id}").update({"priority": new_priority})
                    st.success("✅ Güncellendi.")
                    st.rerun()
            with col2:
                if st.button("🗑️ Sil", key=f"delete_{movie_key}"):
                    ref.child(f"to_watch_firebase/{db_key}/{imdb_id}").delete()
                    st.warning("❌ Silindi.")
                    st.rerun()
            with col3:
                if st.button("📌 Başa Tuttur", key=f"pin_{movie_key}"):
